...
CMP800 = new Controller();

CMP800.currentDeck = function (group) {
    if (group == "[Channel1]")
        return 0;
...
CMP800.currentDeck2 = function (group) {
    if (group == "[Channel1]")
        return "[Channel1]";
    else if (group == "[Channel2]")
        return "[Channel2]";
...
CMP800.escratch = [false, false];

CMP800.UseAcceleration = true;

CMP800.JogSensitivity = 0.5;

CMP800.init = function (channel, control, value, status, group) {
...
CMP800.trackSearch = function (channel, control, value, status, group) {
    if (value == 0x41) {
...
CMP800.menuSearch = function (channel, control, value, status, group) {
    if (value == 0x41) {
...
CMP800.play = function (channel, control, value, status, group) {
    var currentlyPlaying = engine.getValue(CMP800.currentDeck2(group),"play");
    if ((currentlyPlaying == 1) & (value == 0x00)) {
        engine.setValue(CMP800.currentDeck2(group),"play",0);
        midi.sendShortMsg(0x90,0x2A,0x00);
    }
    if ((currentlyPlaying == 0) & (value == 0x00)) {
        engine.setValue(CMP800.currentDeck2(group),"play",1);
        midi.sendShortMsg(0x90,0x2A,0x7F);
        midi.sendShortMsg(0x90,0x30,0x00);
    }
}

CMP800.cue = function (channel, control, value, status, group) {
    var currentlyPlaying = engine.getValue(CMP800.currentDeck2(group),"play");
    if ((currentlyPlaying == 1) & (value == 0x7F)) {
        engine.setValue(CMP800.currentDeck2(group),"cue_default",1);
        midi.sendShortMsg(0x90,0x2A,0x00);
        midi.sendShortMsg(0x90,0x30,0x7F);
    }
        ...
        engine.setValue(CMP800.currentDeck2(group),"cue_default",0);
        midi.sendShortMsg(0x90,0x30,0x7F);
    }
}

CMP800.keylock = function (channel, control, value, status, group) {
    var keylockStat = engine.getValue(CMP800.currentDeck2(group),"keylock");
    if (value == 0x7F & keylockStat == 1) {
        engine.setValue(CMP800.currentDeck2(group),"keylock",0);
        midi.sendShortMsg(0x90,0x06,0x00);
    }
    if (value == 0x7F & keylockStat == 0) {
        engine.setValue(CMP800.currentDeck2(group),"keylock",1);
        midi.sendShortMsg(0x90,0x06,0x7F);
    }
}

CMP800.pitchRateRange = function (channel, control, value, status, group) {
    var pitchRateRange = engine.getValue(CMP800.currentDeck2(group),"rateRange")
    if (value == 0x7F) {
        switch (pitchRateRange) {
            case 0.04:
                engine.setValue(CMP800.currentDeck2(group),"rateRange",0.08);
                midi.sendShortMsg(0x90,0x0C,0x00);
                midi.sendShortMsg(0x90,0x31,0x7F);
                midi.sendShortMsg(0x90,0x32,0x00);
                midi.sendShortMsg(0x90,0x33,0x00);
                ...
                break;
            case 0.08:
                engine.setValue(CMP800.currentDeck2(group),"rateRange",0.16);
                midi.sendShortMsg(0x90,0x0C,0x00);
                midi.sendShortMsg(0x90,0x31,0x00);
                ...
                break;
            case 0.16:
                engine.setValue(CMP800.currentDeck2(group),"rateRange",1.00);
                midi.sendShortMsg(0x90,0x0C,0x00);
                midi.sendShortMsg(0x90,0x31,0x00);
                midi.sendShortMsg(0x90,0x32,0x00);
                ...
                break;
            case 1.00:
                engine.setValue(CMP800.currentDeck2(group),"rateRange",0.04);
...
CMP800.pitchRate = function (channel, control, value, status, group) {
    var pitchRate = engine.getValue(CMP800.currentDeck2(group),"rateRange");
    if ((value == 0x7F) & (pitchRate != 0)) {
        engine.setValue(CMP800.currentDeck2(group),"rateRange",0);
        midi.sendShortMsg(0x90,0x01,0x00);
        midi.sendShortMsg(0x90,0x0C,0x00);
        midi.sendShortMsg(0x90,0x31,0x00);
        midi.sendShortMsg(0x90,0x32,0x00);
...
CMP800.flanger = function (channel, control, value, status, group) {
    var flangerFX = engine.getValue(CMP800.currentDeck2(group),"flanger");
    if ((value == 0x7F) & (flangerFX == 0)) {
        engine.setValue(CMP800.currentDeck2(group),"flanger",1);
        midi.sendShortMsg(0x90,0x2D,0x7F);
    }
    if ((value == 0x7F) & (flangerFX == 1)) {
        engine.setValue(CMP800.currentDeck2(group),"flanger",0);
        midi.sendShortMsg(0x90,0x2D,0x00);
    }

}

CMP800.flangerDelay = function (channel, control, value, status, group) {
    var delayLevel = engine.getValue(CMP800.currentDeck2(group),"lfoDelay");
    if (value == 0x41) {
...
CMP800.wheelTurn = function (channel, control, value, status, group) {
    deck = CMP800.currentDeck(group);
    if (CMP800.escratch[deck]) {
        scratchValue = (value - 0x40);
        engine.scratchTick(deck + 1, scratchValue);
    } else {

        jogValue = (value - 0x40) * CMP800.JogSensitivity;
...
CMP800.wheelTouch = function (channel, control, value, status, group) {
    if (value == 0x7F) {
        deck = CMP800.currentDeck(group);
        CMP800.escratch[deck] = true;
        print(CMP800.escratch[deck]);
        engine.scratchEnable(deck + 1, 100, 330, 1.0/8, (1.0/8)/32);
    }
    else {
        deck = CMP800.currentDeck(group);
        CMP800.escratch[deck] = false;
